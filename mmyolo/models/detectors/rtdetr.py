...
from mmdet.models.layers import RTDETRTransformerDecoder, SinePositionalEncoding
...
class RTDETR(DINO):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        for i in range(batch_inputs.shape[0]):
            ...
            meta_dict = {"batch_input_shape": batch_data_samples['img_metas'][i]['batch_input_shape'],
                         'img_shape': batch_data_samples['img_metas'][i]['batch_input_shape']}
            data_sample.set_metainfo(metainfo=meta_dict)

            single_batch_instance = \
                batch_gt_instances[batch_gt_instances[:, 0] == i, :]
            instance_data.bboxes = single_batch_instance[:, 2:]
            instance_data.labels = single_batch_instance[:, 1]

            data_sample.gt_instances = instance_data
            batch_data_samples_.append(data_sample)
        batch_data_samples = batch_data_samples_
